...
void listadup::ordena() {
    ...
    noh* aux = primeiro;
    noh* nomenor = aux->proximo;
    noh* indice = primeiro;
    acaoPrograma copia = primeiro->acao;
    int menor = primeiro->acao.identificador;

    for(int i = 0; i < tamanho-1; i++) {
        for (int j = i+1; j < tamanho; j++) {
            if (menor > nomenor->acao.identificador) {
                menor = nomenor->acao.identificador;
                indice = nomenor;
            }
            nomenor = nomenor->proximo;
       }

        //troca de conteudo dos nos
        copia = indice->acao;
        indice->acao = aux->acao;
        aux->acao = copia;
        // atualiza os valores para o proximo looping
        aux = aux->proximo;
        indice = aux;
        menor = aux->acao.identificador;
        if(i != tamanho-1) // para nao dar erro
        nomenor = aux->proximo;
    }
